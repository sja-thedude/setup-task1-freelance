...
class CreateCouponRequest extends Request
{
    ...
    public function rules()
    {
        ...
        return array_merge(Coupon::$rules, [
            'code'              => [
                'required',
                'max:255',
                Rule::unique('coupons')->where(function ($query) use ($tmpUser) {
                    return $query->where('workspace_id', $tmpUser->workspace_id)->where('code', $this->code)->whereNull('deleted_at');
                })
            ],
    ...
    public function prepareForValidation()
    {
        ...
        if ($this->has('expire_time')) {
            $inputSwitch['expire_time_valid'] = Carbon::parse($this->expire_time)->format('Y-m-d H:i');
        }
        ...
        foreach ($languages as $langKey => $langLabel) {
            ...
            $langScan['promo_name'] = $this->promo_name;
