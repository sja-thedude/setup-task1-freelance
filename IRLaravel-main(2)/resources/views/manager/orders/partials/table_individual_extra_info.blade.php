    ...
    <div class="row">
        ...
        <div class="col-sm-12 col-xs-12 mgt-10">
            <div class="row">
                ...
                <div class="{!! !empty($optionSettingId) && !$option->items->isEmpty() && !empty($countFlag) ? 'col-sm-7' : 'col-sm-9' !!} col-xs-12">
                    @include('manager.orders.partials.table_extra_info_normal_order')
                </div>

                <div class="col-sm-3 col-xs-12">
                    @if($order->isSelfOrdering())
                        <div class="row mgb-5">
                        ...
                        if(!$order->orderItems->isEmpty()) {
                            foreach($order->orderItems as $orderItem) {
                                if(!empty($orderItem->vat_percent)) {
                                    $totalPrice = (!empty($orderItem->total_price) && $orderItem->total_price > 0) ? $orderItem->total_price : 0;
