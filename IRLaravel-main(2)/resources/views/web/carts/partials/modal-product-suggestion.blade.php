<div class="user-modal hidden" id="modalProductSuggesstion" >
    <div class="bg"></div>
    <div class="check-modal-container">
        <div class="modal-content modal-meal wrap-popup-text">
            <a href="javascript:;" class="close product-suggestion">
                <i class="icn-close"></i>
            </a>

            <div class="check-content">
                <div class="wrap-title">
                    <h2>@lang('cart.title_modal')</h2>
                </div>
                <div class="grid-meal">
                    <div class="wrap-meal">
                        @php $i = 0; @endphp
                        @foreach($productSuggesstions as $product)
                            @php $i++; @endphp
                            <div class="item-meal pro-show-detail" data-locale="{{app()->getLocale()}}"
                                 data-mobile="true"
                                data-id="{{$product['id']}}"
                                data-url="{!! route($guard.'.product.detail', [$webWorkspace->id]) !!}"
                            ...
                            >
                                <div class="wrap-item-content">
                                    <div class="wrap-content">
                                        <h6>
                                            <a href="javascript:;" class="pro-show-detail1"
                                        ...
                                        @if(!empty($product['description']))
                                            <label>{{ str_limit($product['description'], 100, '...') }}</label>
                                        @endif
                                        @if ($product['allergenens'])
                                            <div class="wrap-info">
                                                @foreach($product['allergenens'] as $aller)
                                                    <a href="javascript:;">
                                                        <img width="50" src="{{ str_replace("/gray/", "/hover/", $aller['icon']) }}"/>
                                                    </a>
                                                @endforeach
                                            </div>
                                        @endif
                                    ...
                                    @if ($product['photo'])
                                        <div class="wrap-image">
                                            <a href="javascript:;" class="bd-image">
                                                <img src="{{ $productPhotoPath }}" alt="{{ $product['name'] }}">
                                            </a>
                                        </div>
                                    @else
                                        <div class="wrap-image hide-mobile">
                                            <a href="javascript:;" class="bd-image" style="background: none"></a>
                                        </div>
                                    @endif
                                    ...
                                    <div class="wrap-bottom">
                                        <div class="row">
                                            <div class="col-md-9">
                                                <!-- @foreach($product['labels'] as $lb)
                                                    @if ($lb['type_display'])
                                                        <span class="attr-meal attr-{{ strtolower($lb['type_display']) }}">
                                                            {{ $lb['type_display'] }}
                                                        </span>
                                                    @endif
                                                @endforeach -->
                                                @foreach($product['labels'] as $label)
                                                    @if($label['type_display'] === 'NEWW')
                                                        <span class="attr-meal attr-new">NEW</span>
                                                    ...
                                                    @elseif($label['type_display'] === 'PROMO')
                                                        <span class="attr-meal attr-new">PROMO</span>
                ...
                <div class="text-center mb-20 mt-20">
                    <a href="javascript:;" class="btn btn-andere btn-pr-custom @if(!auth()->guest())btnModalSugesstProduct @else btn-show-login-modal @endif">
                        @lang('cart.btn_modal')
                    </a>
                </div>
