...
const RestaurantFavoriteScreen = () => {
    ...
    const favoriteRestaurantData = useAppSelector((state) => state.restaurantReducer.favoriteRestaurant.data);
    const refreshing = useAppSelector((state) => state.restaurantReducer.favoriteRestaurant.refreshing);
    const loading = useAppSelector((state) => state.restaurantReducer.favoriteRestaurant.loading);
    const canLoadMore = useAppSelector((state) => state.restaurantReducer.favoriteRestaurant.canLoadMore);
    ...
    const getRestaurantRecent = useCallback((loading: boolean, refreshing: boolean) => {
        dispatch(getFavoriteRestaurant(loading, refreshing, params.current));
    }, [dispatch]);
    ...
    useEffect(() => {
        ...
        return () => {
            dispatch(RestaurantActions.clearFavoriteRestaurant());
        };
    }, [dispatch, getRestaurantRecent]);

    const handleNavToDetail = useCallback((item: RestaurantFavoriteItemModel) => {
    ...
    const renderItem = useCallback(({ item } : {item: RestaurantFavoriteItemModel}) => {
                ...
                <View style={styles.imageContainer}>
                    ...
                    <View >
                        ...
                        <TextComponent style={{ ...styles.resFavorite, color: themeColors.color_text_2 }}>
                            {`${item.total_favorite} ${t(item.total_favorite > 1 ? 'text_favorites' : 'text_favorite').toLowerCase()}`}
                        </TextComponent>
                    </View>
                </View>
