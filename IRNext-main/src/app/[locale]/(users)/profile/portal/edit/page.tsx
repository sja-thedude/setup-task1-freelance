...
import Menu from "../../../../components/menu/menu-portal";
...
export default function EditProfile() {
    ...
    const handleEditClick = () => {
        ...
        if (formik.values.first_name === '' || formik.values.last_name === ''
            || formik.values.gsm === '' || formik.values.email === '') {
        ...
        if (event.target.files && event.target.files[0]) {
            ...
            let dataFavorites = api.post(`profile/change_avatar`, data, {
                ...
                setAvatar(res?.data?.data?.photo ?? '/img/avatar.png');
                    toast.dismiss();
                    // Hiển thị toast
                    toast(trans('upload-avatar-success'), {
                        position: toast.POSITION.BOTTOM_CENTER,
                        autoClose: 1500,
                        hideProgressBar: true,
                        closeOnClick: true,
    ...
    return (
        <>
            ...
            <div className={style['navbar']}>
                <div className={style['profile-text']} style={{ fontSize: '30px', display:'flex' }}>
                    <FontAwesomeIcon icon={faChevronLeft} style={{fontSize: '25px'}}
                                     className={`${style['style-icon']} my-auto me-2`} onClick={() => {router.back()}} />
                    <div>
                    ...
                    <ThemeProvider theme={theme}>
                        <form onSubmit={formik.handleSubmit} method={'POST'}>
                            <Grid container spacing={2} style={{ justifyContent: 'center' }}>
                                ...
                                <Grid item xs={12} sm={12}>
                                    <TextField
                                        className={`${variables.texting} ${formik.touched.last_name && (Boolean(formik.errors.last_name) || (apiErrors && apiErrors.last_name)) ? invalid : ''}`}
                                        fullWidth
                                        id="last_name"
                                        name="last_name"
                                ...
                                <Grid item xs={12} className={`d-flex justify-content-center`} style={{ margin: 'auto' }}>
                                    <Button type="submit" onClick={handleEditClick}>
                                        <div
                                        className={`${style['language-save-button']}`}
