...
function LabelList({ labels, color }: { labels: any; color: string }) {
  const trans = useI18n();
  const settings = {
    infinite: false,
    variableWidth: true,
  ...
  const cateProFrom = searchParams.has("from");

  return (
    <>
      {cateProFrom ? (
        <div className="label-list d-flex" style={{ width: "max-content" }}>
          {labels?.map(
            (label: label, i: number) =>
              label.type != 0 && (
                <div className="label-item me-2" key={"label-" + label.id}>
                  <div
                    className={`attr-meal ${style["attr-meal"]} ${
                      style["attr-" + label.type]
                    }`}
                    style={label.type > 3 ? { background: color } : {}}
                  >
        ...
        <div className="label-list">
          <Slider {...settings}>
            {labels?.map(
              (label: label, i: number) =>
                label.type != 0 && (
                  <div className="label-item" key={"label-" + label.id}>
                    <div
                      className={`attr-meal ${style["attr-meal"]} ${
                        style["attr-" + label.type]
                      }`}
