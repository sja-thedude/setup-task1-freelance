...
export default function PortalRegister({ togglePopup, toggleReadyPopup , setToggleLoginPopUp}: { togglePopup: any, toggleReadyPopup: any , setToggleLoginPopUp: any}) {
    ...
    const [errorMessageEmail, setErrorMessageEmail] = useState<any | null>(null);
    const [errorMessageGSM, setErrorMessageGSM] = useState<any | null>(null);
    const [errorMessagePass, setErrorMessagePass] = useState<any | null>(null);
    ...
    function checkEmailValid(email: string) {
        // use regex to check email valid
    ...
    const handleSubmitClick = () => {
        ...
        if (formik.values.password.length < 6 && formik.values.password.length > 0) {
            setErrorMessagePass(trans('password-min-length'));
        ...
        if (formik.values.gsm.length < minGsm && formik.values.gsm.length > 0) {
            setIsGsmValid(false);
            setErrorMessageGSM(trans('format-gsm'));
        } else {
            setErrorMessageGSM('');
        }
        if (formik.values.first_name === '' || formik.values.last_name === '' || formik.values.gsm === '' || formik.values.email === '' || formik.values.password === '' || formik.values.password_confirmation === '') {
            ...
            if (checkEmailValid(formik.values.email)) {
                ...
                setIsEmailValid(false);
                if (formik.values.email) {
                    setErrorMessageEmail(trans('email-fail'));
                }
            }
        } else {
            if (checkEmailValid(formik.values.email)) {
            ...
            if (formik.values.password !== formik.values.password_confirmation) {
                setErrorMessagePass(trans('password-not-match'));
                setIsPasswordValid(false);
                setIsPasswordConfirmationValid(false);
            } else {
                setErrorMessagePass('');
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            if (isEmailValid && isFirstNameValid && isLastNameValid && isGsmValid && !isFirstIconVisible && isPasswordValid && isPasswordConfirmationValid && values.password.length >= 6 && values.password_confirmation.length >= 6) {
                toast.dismiss();
                try {
                    const headers = {
    ...
    const handleItemClick = (item: any) => {
        ...
        formik.setFieldValue('postcode', `${item.postcode}, ${item.address ? item.address : ''} ${item?.city?.name}`);
    }
    ...
    const responseLoginToken = (response: any, provider: string) => {
        let data = api.post(`login/social`, {
            ...
            if (userData?.first_login && (userData.first_name.includes('@') || config.REGEX_NUMBER_CHECK.test(userData.first_name) || !userData.gsm)) {
    ...
    const handlePass = () => {
        ...
        setIsPasswordValid(true)
        const updateApiErrors = { ...apiErrors }
        delete updateApiErrors.password
        setApiErrors(updateApiErrors)
    }
    const handlePassConfirm = () => {
        setIsPasswordConfirmationValid(true)
        setCurrentPasswordConfirmation(formik.values.password_confirmation)
        const updateApiErrors = { ...apiErrors }
        delete updateApiErrors.password_confirmation
        setApiErrors(updateApiErrors)
    }
    ...
    return (
        <>
            ...
            <Modal show={show} onHide={handleClose}
                ...
                centered id='portal-register'
            >
                <Modal.Body >
                    <div className='header-contain'>
                        <p className='back-step-portal' onClick={handleBackStep}>{`< ${trans('back')}`}</p>
                        <h3 >{trans('register-account')}</h3>
                    </div>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" className='quitting' onClick={handleClose}>
                        <path d="M18 6L6 18" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        <path d="M6 6L18 18" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                    <ThemeProvider theme={theme}>
                        <div className='row' onClick={() => { setIsShow(false) }}>
                            <form onSubmit={formik.handleSubmit}>
                                <Grid container spacing={2} style={{ justifyContent: 'center' }}>
                                    ...
                                    <Grid item xs={9} style={{ padding: '0', paddingLeft: '16px' }}>
                                    ...
                                    <Grid item xs={9}>
                                        <TextField
                                            ...
                                            id="email"
                                            name="email"
                                            label={formik.values.email ? '' : ' ' + trans('lang_email_address')}
                                            variant="outlined"
                                    ...
                                    {errorMessage ? (
                                        <Grid item xs={12} className={`${variables.regis} d-flex justify-content-center mt-2`} style={{ margin: 'auto' }}>
                                            <div className={`${variables.bv} ms-2`}>
                                                {errorMessage}
                                            </div>
                                        </Grid>
                                    ) : null}
                                    ...
                                    <Grid item xs={9} className={`${variables.regis} d-flex justify-content-between`} style={{ margin: 'auto', paddingTop: '0px' }}>
                                        <GoogleLogin
