...
export default function Page() {    
    ...
    const handleEnableRestaurant = () => {
        if (statusEnableTableOrdering === 0) {
            window.location.href = '/self-ordering';
        } else {
    ...
    const [typeOpeningHours, setTypeOpeningHours] = useState<any | null>(3);
                ...
                <div className={`${variables.heying} heying row relative-index-2 text-center justify-content-center ps-2 pe-2`}>
                    {statusEnableTableOrdering === 0 ? (
                        <div className={`row justify-content-center ${variables.table_ordering_notfound}`}>
                            <div style={{ padding: '0' }}>
                                <h1>{trans('self-ordering-notfound-title')}</h1>
                                <p>{trans('self-ordering-notfound-description')}</p>
                                <div className={`${buttonOrder} ${variables.button} mt-5`}>
                                    <button type="button" className={btnDark} onClick={handleEnableRestaurant}>{trans('self-ordering-notfound-button')}</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className={`row justify-content-center ${variables.table_ordering_notfound}`}>
                            <div style={{ padding: '0' }}>
                                ...
                                <h1>{trans('self-ordering-notfound-closed-title')}</h1>
                                <p>{trans('self-ordering-notfound-closed-description')}</p>
                                <div className={`${buttonOrder} ${variables.button} mt-5`}>
                                    <button type="button" className={btnDark} onClick={handleEnableRestaurant}>{trans('self-ordering-notfound-closed-button')}</button>
                                </div>
                            </div>
                        </div>
                    ) : true ? (
                        <div className={`${variables.table_ordering_open}`}>
                            <div>
                                ...
                                <div className={`${buttonOrder} ${variables.buttonOrder} mt-3`} style={{ margin: 'auto' }}>
                                    <div onClick={handleGoTable} >
                                        <a>
                                            <button type="button" className={`${btnDark} text-uppercase`}>
                                                {_.isEmpty(selfOrderingCart) ? trans('start-order') : trans('order_continuous')}
                                            </button>
                                        </a>
                                    </div>
                                </div>
